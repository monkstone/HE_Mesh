...
public class WB_GeometryOp2D extends WB_GeometryOpGLU {
	...
	public static final ArrayList<WB_Point> getIntersection2D(final WB_Circle C0, final WB_Circle C1) {
		...
		if (!WB_Epsilon.isZeroSq(WB_GeometryOp2D.getSqDistance2D(p0, p1))) {
	...
	public static final ArrayList<WB_Point> getIntersection2D(final WB_Line L, final WB_Circle C) {
		...
		final double c = WB_GeometryOp2D.getSqLength2D(C.getCenter()) + WB_Vector.getSqLength2D(L.getOrigin())
	...
	public static final ArrayList<WB_Point> getIntersection2D(final WB_Ray R, final WB_Circle C) {
		...
		final double c = WB_GeometryOp2D.getSqLength2D(C.getCenter()) + WB_Vector.getSqLength2D(R.getOrigin())
	...
	public static final WB_Point getClosestPoint2D(final WB_Coord p, final WB_PolyLine PL) {
		...
		for (int s = 0; s < PL.getNumberSegments(); s++) {
			...
			d2 = WB_GeometryOp2D.getSqDistance2D(p, test);
			if (d2 < d2min) {
				d2min = d2;
				q = test;
			}
	...
	public static final WB_IntersectionResult getClosestPoint2D(final WB_Segment S1, final WB_Segment S2) {
		...
		i.sqDist = WB_GeometryOp2D.getSqDistance2D(p1, p2);
	...
	public static final WB_IntersectionResult getClosestPoint2D(final WB_Line L1, final WB_Line L2) {
		...
		if (WB_Epsilon.isZero(denom)) {
			...
			final double d2 = WB_GeometryOp2D.getSqDistance2D(L1.getOrigin(), p2);
			final WB_IntersectionResult i = new WB_IntersectionResult();
			i.intersection = false;
			i.t1 = 0;
			i.t2 = t2;
		...
		final WB_Point p2 = new WB_Point(L2.getPoint(t2));
		final double d2 = WB_GeometryOp2D.getSqDistance2D(p1, p2);
		final WB_IntersectionResult i = new WB_IntersectionResult();
		i.intersection = true;
		i.t1 = t1;
		i.t2 = t2;
	...
	public static final WB_Point getClosestPointOnPeriphery2D(final WB_Coord p, final WB_Triangle T) {
		...
		final double dB2 = WB_GeometryOp2D.getSqDistance2D(p, B);
		final WB_Point C = getClosestPointToSegment2D(p, T.p1, T.p2);
		final double dC2 = WB_GeometryOp2D.getSqDistance2D(p, C);
		if (dA2 < dB2 && dA2 < dC2) {
			return A;
	...
	public static final WB_Point getClosestPoint2D(final WB_Coord p, final WB_Polygon poly) {
		...
		for (int i = 0; i < n; i += 3) {
			...
			final double d2 = WB_GeometryOp2D.getDistance2D(tmp, p);
			if (d2 < dmax2) {
				closest = tmp;
				dmax2 = d2;
			}
	...
	public static final WB_Point getClosestPointOnPeriphery2D(final WB_Coord p, final WB_Polygon poly) {
		...
		for (int i = 0; i < n; i += 3) {
			...
			tmp = getClosestPointToTriangle2D(p, poly.getPoint(tris[i]), poly.getPoint(tris[i + 1]),
					poly.getPoint(tris[i + 2]));
			final double d2 = WB_GeometryOp2D.getSqDistance2D(tmp, p);
			if (d2 < dmax2) {
				closest = tmp;
				dmax2 = d2;
			}
	...
	public static final boolean between2D(final WB_Coord a, final WB_Coord b, final WB_Coord c) {
		if (WB_GeometryOp2D.isCoincident2D(a, c)) {
			return true;
		} else if (WB_GeometryOp2D.isCoincident2D(b, c)) {
			return true;
		} else {
	...
	public static final WB_Coord getSegmentIntersection2D(final WB_Coord ap1, final WB_Coord ap2, final WB_Coord bp1,
		...
		double BxA = WB_GeometryOp2D.cross2D(B, A);
		if (Math.abs(BxA) <= WB_Epsilon.EPSILON) {
			return null;
		}

		double ambxA = WB_GeometryOp2D.cross2D(WB_Point.sub(ap1, bp1), A);
	...
	public static final boolean getLineIntersectionInto2D(final WB_Coord a1, final WB_Coord a2, final WB_Coord b1,
		...
		double det = WB_GeometryOp2D.cross2D(s1, s2);
		if (Math.abs(det) <= WB_Epsilon.EPSILON) {
			return false;
		} else {
			det = 1.0 / det;
			double t2 = det * (WB_GeometryOp2D.cross2D(a1, s1) - WB_GeometryOp2D.cross2D(b1, s1));
