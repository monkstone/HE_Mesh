	...
	public int[] getAlphaTriangles(final double a) {
		...
		for (int i = 0; i < tetrahedra.length; i += 4) {
			if (alpha[i / 4] <= a || Double.isNaN(alpha[i / 4])) {
				...
				key = new Key(tetrahedra[i + 1], tetrahedra[i], tetrahedra[i + 3]);
				...
				key = new Key(tetrahedra[i + 2], tetrahedra[i + 1], tetrahedra[i + 3]);
				...
				key = new Key(tetrahedra[i], tetrahedra[i + 2], tetrahedra[i + 3]);
